...
/**
 * Helper methods related to requesting and receiving data from USGS.
 */
public final class QueryUtils {
    ...
    private static List<BookListing> extractFeatureFromJson(String BookListingJSON) {
        ...
        try {
            ...
            // which represents a list of items (or bookListings).
            JSONArray BookListingArray = baseJsonResponse.getJSONArray("items");

            // For each earthquake in the BookListingArray, create an {@link BookListing} object
            for (int i = 0; i < BookListingArray.length(); i++) {
                ...
                Log.d("QueryUtils", "Value: " + Float.toString(number));
                /*// Extract the value for the key called "authors"
                JSONArray authors = volumeInfo.getJSONArray("authors");*/

                // Extract the value for the key called "description"
                String description = volumeInfo.getString("description");
                if ( volumeInfo.getString("description").isEmpty())
                {
                    description= "there is no description here";
                }
                // Extract the value for the key called "infoLink"
                String url = volumeInfo.getString("infoLink");
                // Create a new {@link BookListing} object with the title, description,
                // and url from the JSON response.
                BookListing bookListing = new BookListing(title, description, url);
                number++;
            ...
            Log.e("QueryUtils", "Problem parsing the Book Listing JSON results", e);
        }
